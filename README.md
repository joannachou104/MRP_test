...
建議的測試流程(對應規格書第4節模組)
商品管理 / BOM 管理:若未執行 seed,可先手動建立商品與 BOM,或用批次匯入(新增/更新兩介面各自的 CSV/Excel 範本欄位見畫面說明)。
通路管理:建立通路(大宗/飯店/B2C),於「資料匯入中心」上傳大宗季預測、飯店月預測。
資料匯入中心:上傳銷貨/銷退歷史(供 B2C 近3月均量+YOY 計算)、每日庫存快照。
MRP 運算儀表板:點選「執行 MRP 運算」,即可看到淨需求、可銷售天數、缺貨警示。
缺口追溯:選一個組合品/成品,沿 BOM 逐層往下看每個節點的缺口狀態。
採購建議:對觸發缺口的商品「登記在途」或「忽略」;在「在途清單」分頁確認到貨(支援部分到貨)。
忽略清單複核:查看目前被忽略的商品,或取消忽略。
加工配送管理:建立加工廠/包裝廠、匯入委外加工排程(ERP 匯出格式),輸入原料/成品可分配量產生配送清單。
系統設定/權限:查看角色權限說明與異動紀錄(交接用)。
專案結構
```
backend/
  app/
